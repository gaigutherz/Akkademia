...
def translate_from_transliteration(file):
    tokenize("transliteration_bpe", file, False, Path("NMT_input/tokenization"), Path(""), Path("/tmp"), True)
...
def translate_from_akkadian(file):
    tokenize("signs_char", file, False, Path("NMT_input/tokenization"), Path(""), Path("/tmp"), True)
    cmd = "../fairseq/fairseq_cli/interactive.py " \
